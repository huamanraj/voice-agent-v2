...
Tips to reduce Nova-3 latency on WebSocket:
Use interim_results=true — interim transcripts have the lowest latency; avoid measuring latency on final results [GitHub discussion]
Send audio in ~20ms chunks [Discord thread]
Use a sample rate of 16kHz (higher rates like 48kHz increase latency) [Discord thread]
Lower the endpointing value for faster speech_final responses [Discord thread]
Avoid features like diarization or smart formatting if latency is critical 
